---
path: '/jobs'
date: '2023-10-09'
title: 'Job Openings (Vacancies)'
---

## PhD and Research Assistant positions

**Research Assistant (f/m/d)**
  -  Code 6127: Knowledge Graphs, e.g. SPARQL, embeddings and optimization
  -  full-time position (salary group E13 TV-L)
  -  Design and implementation of machine learning algorithms for knowledge graphs and large lan-guage models
  -  Deployment of algorithms at large scale and on HPC infrastructure
  -  Design and evaluation of a benchmarking environment
  -  Evaluation of models on down-stream tasks
  -  [PDF for reference no. 6127](https://www.uni-paderborn.de/fileadmin/zv/4-4/stellenangebote/Kennziffer6127_-_nur_Englisch.pdf)

**Research Assistant (f/m/d)**
  -  Code 6128: Machine Learning, e.g. Deep Learning, Reinforcement Learning, Preference Learn-ing, MLOps 
  -  full-time position (salary group E13 TV-L)
  -  Design and implementation of machine learning algorithms for knowledge graphs and large lan-guage models
  -  Deployment of algorithms at large scale and on HPC infrastructure
  -  Design and evaluation of a benchmarking environment
  -  Evaluation of models on down-stream tasks
  -  [PDF for reference no. 6128](https://www.uni-paderborn.de/fileadmin/zv/4-4/stellenangebote/Kennziffer6127_-_nur_Englisch.pdf)

**Research Assistant (f/m/d)**
  -  Research on knowledge graph robustness
  -  full-time position (salary group E13 TV-L)
  -  Contributions to the SAIL project results, to international conferences and written publications
  -  Collaboration in the activities of the SAIL network
  -  [PDF for reference no. 6107](https://www.uni-paderborn.de/fileadmin/zv/4-4/stellenangebote/Kennziffer6107_-_nur_Englisch.pdf)

### Application

To apply for a PhD position:

1. Send your request to [Simone Auinger](/SimoneAuinger) (<a href="mailto:simone.auinger@uni-paderborn.de">simone.auinger@uni-paderborn.de</a>).  
   Inquiries pertaining to technical positions can be also directed to [Michael Röder](/MichaelRoeder) (<a href="mailto:michael.roeder@upb.de">michael.roeder@upb.de</a>).
2. E-mail subject (replace the __Job-ID__ with the reference number of the position): Application for __Job-ID__.
3. Please use PDF for attachments.


### Requirements

Excellent candidates are invited to apply with:

1. Curriculum vitae and copies of degree certificates/transcripts,
2. Writing samples/copies of relevant scientific papers (e.g. thesis),
3. A short statement of research and/or technology interest,
4. Letters of recommendation.


## Student Assistant positions

We offer student assistant positions to work with us on research and teaching projects (SHK/WHB with up to 19h/week). Available positions will be listed here.

* [Java developer in benchmarking projects](/jobs/StudentAssistant-Java-Benchmarking)
* [Python developer for Knowledge Graph-driven Semantic Search](/jobs/StudentAssistant-Python-LASSKG)

To apply for one or several assistant positions, please fill our <ExternalLink to="https://umfragen.uni-paderborn.de/index.php/143257">application form</ExternalLink>.

## Additional positions at Paderborn University

<ExternalLink
     to="https://www.uni-paderborn.de/en/zv/4-4/stellenangebote"
   >Vacancies at Paderborn University</ExternalLink> and <ExternalLink
     to="https://www.uni-paderborn.de/en/zv/4-4/stellenangebote/stellenangebotewieim"
   >scientific employees at the Faculty of Computer Science, Electrical Engineering and Mathematics</ExternalLink>
